...
- 默认 Qwen + Hybrid 生成有证据引用的建议；API Key 可由界面仅写入服务内存。规则只在未配置、调用失败或校验失败时安全回退。模型不直接发送消息、付款或修改外部工单。
...
4. 默认 Qwen + Hybrid、缓存与来源/语义状态校验；失败时显式安全回退，不伪称模型结果。
...
待填写：队伍名称与成员；真实模型凭据由界面内存配置或本机环境变量提供。
