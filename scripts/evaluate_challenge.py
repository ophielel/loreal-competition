...
def main():
    ...
    for case in dataset['cases']:
        ...
        actionable_expected=expected_state in ('实际发生','待核实','既往情况')
        actionable_actual=any(s['actionable'] for s in signals)
        ...
        rows.append({'id':case['id'],'intent_ok':actual['intent']==expected['intent'],
                     ...
                     'negative_risk_case':expected_state in (None,'否定','假设/咨询'),
    ...
    lines += ['','Evidence Support 在本集合中要求引用同一条语义支持消息，不以“ID 合法”替代语义审核。','该小集合用于冲刺边界回归，不宣称生产泛化或盲测成绩。']
